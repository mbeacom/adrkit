...
- [ ] T023b [P] [US3] [OBSERVE-FAIL] Fixture + test: the `not-computable` class is **derived from observed state**, never hardcoded per trigger (FR-023a, SC-031). Observe failing against an implementation containing `if (trigger === 'novel-no-precedent') return 'nothing-to-measure'`. **The fixture MUST be a synthetic input to the classifier, not a repository state** — a state in which a relevance primitive is recorded present and its output empty, handed directly to the pure classifier. That distinction is load-bearing and was wrong here once: expressed as a repository state this observation is **impossible**, because FR-020d establishes that no relevance scoring exists anywhere and `specs/011-*` deliberately builds no ranker, so `novel-no-precedent` is `evidence-absent` for every constructible case and the hardcoded implementation returns the *correct* class for every input that can exist today. A test that cannot distinguish the hardcode from a correct implementation **passes vacuously while reporting as ADR-0016 coverage**. The classifier is a pure function over a state type this feature defines (T008), so the distinguishing input exists at the unit boundary even though no primitive ships — which is the whole reason the observation is recoverable rather than deferred
...
- [ ] T051b **Withdrawn-design scan.** When a design is withdrawn or replaced, scan for the **mechanism's own vocabulary** — and for descriptions of its *behavior*, which contain none of its words — never for the requirement's ID. Reference integrity passes cleanly on this class while the prose disagrees. Two rules travel with it: state the withdrawal **where the old design was argued**, because rationale sections outlive requirement edits; and keep the superseded reasoning attached as its own counterexample, so a reader who wonders why the obvious approach is unused finds the answer instead of proposing it again. **This check exists because this feature kept being bitten by it; no count is given, because a tally of recurrences is a fact about a drafting session and must be maintained to stay true, and this one was already stale when T051c recorded a further instance.** Known episodes: first leaving `plan.md`'s design-rationale section still arguing for the withdrawn dependency-graph detector, then leaving four more references that described the mechanism by what it *did* rather than by its name, including SC-006, which enumerated a deliberate violation that could no longer occur

- [ ] T051c **Extend T051a and T051b to the pull-request description, and to any other prose that argues for the design but is not a tracked file in this directory.** A scan scoped to `spec.md` / `plan.md` / `tasks.md` is narrower than the claim "the withdrawn design is gone", because the PR description is where the design is *argued to a reviewer* and is the first thing read and the last thing re-read. **This task exists because that gap was real and load-bearing:** during scoping, PR #141's description still described the detector as "a declared registry cross-checked against `check:deps`" — the withdrawn design, presented as current — for the entire review, while every scan over the three committed files came back clean. The same pass found all six numbers in its file table stale (`1,337`/`42 FRs`/`38 SCs`/`372`/`375`/`30 [OBSERVE-FAIL]` against actual `1,930`/`52`/`53`/`422`/`417`/`36`), each measuring a superseded revision. Two sub-rules, both observed: **a count stated as an identifier range is a distinct defect** — 52 FRs whose highest identifier is `FR-030` is not "FR-001–FR-052", and a reader who greps `FR-052` to verify finds nothing and concludes requirements are missing; and **an unversioned description silently ages**, so any count in it either cites the revision it was measured at or is derived rather than transcribed. The `[OBSERVE-FAIL]` figure had to be counted twice to get it right: a bare `grep -c OBSERVE-FAIL` returns the legend line and this task's own quotation of the token alongside the 36 marked tasks, so the first replacement number was wrong in the same way the number it replaced was — **a count must match the property, not the token**, which is T051a's rule arriving in numeric form. **A third sub-rule, from the same family: disclose that something recurred, never how many times.** A record hiding its own revision is what this feature argues against, but a disclosure carrying a *tally* — "hit three times", "bitten twice" — is a fact about a drafting session rather than about the artifact, and it goes stale the moment the session continues. Both tallies written here were already false when found: one was contradicted by T051c's own later instance. State the recurrence and the named episodes; omit the count
- [ ] T051e **Verify every edit against the file it targeted, never against the exit status of the tool that made it — and re-verify each requirement body after any multi-edit run that did not complete cleanly.** A scripted edit that builds several changes in memory and writes at the end discards **all** of them when it aborts partway, while the surrounding report still describes them as made. **This class is invisible to every check in this repository**: the requirement still exists, every identifier still resolves, `adr lint` passes, and a cross-reference audit passes, because *reference integrity cannot see a missing paragraph*. Unlike the withdrawn-design and assertion classes, there is no contradictory text to catch by reading — the content is simply absent and nothing in the document points at its absence. Two episodes, one in each concurrent feature: here, a script made five edits, aborted on the sixth assertion, discarded all five, and the commit message described them as done; in `specs/011-*`, an entire FR-023b consumption clause was reported as landed in a commit that never contained it, found only by bisecting for the text rather than for the requirement ID. **The check is textual and per-claim**: after any edit, grep the file for the property's own wording and confirm the count changed as intended — and when auditing whether something landed, search for what it *says*, never for the identifier that would still resolve without it
- [ ] T051d **Impossibility audit of every deliberate-violation fixture — re-run whenever a design any fixture depends on changes.** An `[OBSERVE-FAIL]` task whose named violation can no longer occur produces a fixture that satisfies ADR-0016 **vacuously while reporting as coverage**. In a project whose central discipline is *a check counts only once observed failing*, that artifact is the most dangerous one available: it is **structurally indistinguishable from a working guard**, and an implementer would build it by doing exactly what the document said. The trigger is not decay but **design change** — a fixture does not rot on its own, it rots when the thing it violates is replaced. **Both instances here were produced that way:** SC-006 enumerated a violation of the withdrawn `check:deps` detector that could no longer occur once FR-013 replaced it, and T023b named a fixture requiring a relevance primitive that FR-020d had established does not exist, so the hardcoded implementation it was meant to catch returned the correct class for every constructible input. **A vocabulary scan does not find these** — T023b contained the word, SC-006 did not; the question is whether the violation is *constructible*, which only reading answers. Audit scope when this was first run: all 36 `[OBSERVE-FAIL]` tasks read individually against the current design; one defect (T023b), one prior instance already fixed (SC-006), and the fixtures referencing not-yet-existing `specs/011-*` types (T037, T043c) confirmed **constructible by design**, since this feature defines their shapes in `contracts/metric-definitions.md` (T008) and they are synthetic inputs to pure functions rather than repository states
...
- **Task-ID ranges in this document are inclusive of suffixed IDs that fall
  ...
  and so on. Twenty-seven task IDs carry a letter suffix, so the number of tasks
  (77) is larger than the highest task number, and a range is **not** a count.
