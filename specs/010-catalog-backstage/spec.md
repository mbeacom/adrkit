...
- **FR-050**: A clean clone MUST build, typecheck, lint, and test green. No post-install step
  ...
  **only** during dependency installation", which is a claim about *availability* and is
  ...
  Sixteen of the seventeen post-install steps additionally run under a **proved** denial
  (`scripts/run-network-denied.ts`). The seventeenth is `bun test`, which cannot: the suite
