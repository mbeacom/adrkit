...
**What that costs.** **Sixteen of the seventeen post-install steps run under a proved
denial; `bun test` is the seventeenth.** Never "every step is denied" — rounding the count
...
`.github/workflows/ci.yml`'s `clean-clone-builds` runs **sixteen of its seventeen
post-install steps** through `scripts/run-network-denied.ts`. The seventeenth is `bun test`,
