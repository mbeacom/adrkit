...
| `envelopes` | reference | `{ communityPlugins: ArtifactFileReference; rhdhPlugins: ArtifactFileReference; synthetic: ArtifactFileReference }` | The three required FR-022 envelopes (`snapshot-envelope.*.json`), by relative path + digest — never merged, never embedded. **Fail-closed-rejection exception**: `ArtifactFileReference` only constrains the *reference* (relative path + digest of some real, hash-verified file) — it does not itself assert that the referenced file's content is §9-shaped. When a pass's derivation deterministically and correctly fail-closed-rejects per ADR-0012/0013 (e.g. a genuine `duplicate-canonical-id` in the real, frozen corpus) such that **no** populated §9 `SnapshotEnvelope` can be honestly produced for that pass, the field is still populated — with a reference to a file that instead documents the rejection outcome (a distinctly-shaped, non-`SnapshotEnvelope` record: what triggered it, why, and that SC-001/SC-002 still held for the aborted run) rather than fabricating or force-fitting a `SnapshotEnvelope` shape that was never actually producible. This is the exact, anticipated mechanism behind `Verdict.blockedShortfall = "envelope-or-scale-evidence-incomplete"` (§23; `contracts/evidence-bundle-and-verdict.md` §2 Step 3, "the envelope ... could not be fully populated from an actual run for any pass") — it is a **named, non-unsafe shortfall**, not a missing/incomplete field under §22's own completeness rule below. |
