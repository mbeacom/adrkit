...
| `authoritativeGoDistinctionStatement` | `string` (fixed template) | Present on **every** verdict regardless of `outcome`: states that none of the three outcomes is, or substitutes for, the hardened contract's "authoritative `go`" status or external / community validation (ADR-0014 rung 3) (SC-012/FR-025). |
...
| `authoritativeGoDisclaimer` | `string` (fixed template) | States explicitly that this recommendation, even if `outcome === "go-explicit"`, does not itself satisfy external / community validation (ADR-0014 rung 3), optional later external-adopter production-maturity evidence, or the hardened contract's "authoritative `go`" status (FR-025/FR-026). |
