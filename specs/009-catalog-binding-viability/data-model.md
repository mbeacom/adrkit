...
| `envelopes` | reference | `{ communityPlugins: EnvelopeOrRejectionRef; rhdhPlugins: EnvelopeOrRejectionRef; synthetic: EnvelopeOrRejectionRef }` | The three required FR-022 envelope slots (`snapshot-envelope.*.json`), by relative path + digest — never merged, never embedded. **`EnvelopeOrRejectionRef` = `ArtifactFileReference & { contentShape: "snapshot-envelope" \| "fail-closed-rejection-record" }`.** The added `contentShape` discriminator lets a consumer determine, **from the bundle alone** (no need to open and sniff the referenced file), which of exactly two schemas the referenced file uses: `"snapshot-envelope"` means the file is §9-shaped and safe to run through §9's validation-before-derivation steps unchanged; `"fail-closed-rejection-record"` means the file instead documents why that pass's derivation deterministically and correctly fail-closed-rejected per ADR-0012/0013 (e.g. a genuine `duplicate-canonical-id` in the real, frozen corpus) — what triggered it, why, and that SC-001/SC-002 still held for the aborted run — and MUST NOT be parsed as a §9 `SnapshotEnvelope`. A `contentShape` of `"fail-closed-rejection-record"` for one or more of the three slots is the exact, anticipated mechanism behind `Verdict.blockedShortfall = "envelope-or-scale-evidence-incomplete"` (§23; `contracts/evidence-bundle-and-verdict.md` §2 Step 3, "the envelope ... could not be fully populated from an actual run for any pass") — it is a **named, non-unsafe shortfall**, not a missing/incomplete field under §22's own completeness rule below, and the discriminator makes the two cases mechanically distinguishable rather than requiring the consumer to infer the shape by fabricating or force-fitting one. |
