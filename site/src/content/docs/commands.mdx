...
- **Any language.** adrkit does not parse your file. A marker counts when one of
  `//`, `/*`, `*`, `#`, `--`, `;`, `%`, `<!--`, `"""`, `'''` appears earlier on
  the same line. The trade-off is honest: `log("// @adr 0012")` inside a string
  reads as a marker, and a marker on a bare continuation line inside a
  `/* … */` block does not.
...
`state` is `scanned`, `absent` (no such file), or `unreadable`. `truncated` is
`true` when the file is longer than the header window, so a marker below it was
never seen. In `governedBy`, a pattern match carries `firedMatchers` and a file
declaration carries `declaredBy`; `declaredBy` is omitted entirely when nothing
declared the record.
