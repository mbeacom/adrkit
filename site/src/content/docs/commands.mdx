...
- **Only the first 8192 bytes** of a file are scanned. A marker is a claim that
  ...
  that decision.
- **Nothing enters the record.** The edge is discovered when you run `explain`.
...
against. An absolute path, one that climbs out of the working tree, or a symlink
whose target is outside it is reported as `out-of-tree` and not read — a file
elsewhere on disk cannot declare itself governed by this corpus.

`adr check` and the CI Action do **not** scan markers — that is a separate
decision, because it changes what CI enforces.
