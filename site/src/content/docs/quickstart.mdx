...
<Aside type="tip" title="Published on npm (v0.14.0)">
...
<Aside type="tip" title="Which record should be first?">
	The decision worth recording first is usually the one you just made by reading
	this page: keep architecture decisions in git, and enforce them with adrkit.
	It governs `docs/adr/**`, so it is the record that explains the directory to
	everyone who finds it later. Write it with
	`adr new "Record architecture decisions as versioned markdown in git"` — it
	then takes `0001`, and the worked example below becomes `0002`. The
	[agent plugin](/backfill/) offers this record when `/adr-backfill` runs
	against a repository with no corpus.
</Aside>
...
- Evaluate the proposed generated-wiki boundary — see
  [Use with generated knowledge systems](/generated-knowledge/) (provisional,
