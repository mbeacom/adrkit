...
<Aside type="tip" title="Already have MADR records?">
	...
	first. adrkit requires `id`, `title`, `status`, and `date`, and rejects unknown
	keys, so linting an unmigrated corpus reports errors on every record. `adr
	migrate` adds the fields in place and is additive.
</Aside>
