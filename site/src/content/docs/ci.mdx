...
Every pull request in the adrkit repository runs this Action against itself and asserts
the result, so the comment path has continuous end-to-end coverage. It is **not yet
reference-verified** on [ADR-0014](/adr/) rung 2 — unlike the queue Action below — and
that gap is tracked openly rather than implied away.
