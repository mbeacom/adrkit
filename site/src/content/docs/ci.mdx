...
<Aside type="note" title="GitHub Marketplace">
	...
	GitHub lists only root Action metadata, so the queue Action remains nested.
</Aside>
...
The Marketplace form is pinned to the immutable `v0.14.0` release. For
...
<Aside type="caution" title="Pinning a release tag by SHA">
	The `v*` release tags are **annotated**, so `refs/tags/v0.14.0` resolves to a tag
	...
	gh api repos/mbeacom/adrkit/commits/v0.14.0 --jq .sha
	```
