...
| 7 — Spec Kit hook viability | `specs/008-spec-kit-hook-viability/` | **executed, out-of-contract on one blocking gate** (T001–T058 run; 56 complete, **T005** and **T012** deliberately left unchecked — see below); verdict **`no-go`** (trigger: `mutation`, independently driven by the `install` and `remove` lifecycle steps, so the verdict itself is unaffected by the gate issue below); T005's rank-1 network-denial mechanism was available but not selected for the recorded lifecycle invocations, so T012 — whose sole defined action is certifying that its named dependency set (including T005) is satisfied — could not honestly close; per PR review round 12, `specs/008-spec-kit-hook-viability/tasks.md`'s own T012 text ("No User Story task below may begin until this checkpoint is confirmed") is an explicit blocking rule, and the User Story tasks that already ran (T013 onward) necessarily began without that rule's own precondition genuinely holding — **this run is therefore disclosed as out-of-contract with respect to that blocking gate, not a fully gate-conformant execution**, even though each User Story task's own distinct, literally-described action (recording a transcript, checking a mutation baseline, validating bundle-field existence, etc.) did run and remains independently evidenced (individual task checkboxes T013–T058 remain checked on that narrower basis) — see [`specs/008-spec-kit-hook-viability/tasks.md`](specs/008-spec-kit-hook-viability/tasks.md)'s T012 note (round 12 addendum) and [`specs/008-spec-kit-hook-viability/checklists/evidence-index.md`](specs/008-spec-kit-hook-viability/checklists/evidence-index.md) for the full accounting; **the 008→009 sequencing precondition below ("execute feature 008 end-to-end first") is therefore NOT established as satisfied by this row** — treating it as satisfied, or requiring a conformant rerun of the live-Copilot lifecycle after fixing T005/T012 first, is a maintainer decision at the time feature 009 is next considered, not settled by this ledger entry; independently audited on a **consistency check whose own scope is genuinely ambiguous between a narrow reading (items (a)–(f) only) and a broader reading (full FR/SC/contract substantive compliance, per PR review round 15) — this ledger entry does not assert the narrow reading is self-evidently correct** (fresh-context, six cumulative audit rounds against the original 6-entry `mutationBaselines` corpus, final PASS on evidence-bundle integrity against the checks each pass was actually dispatched to run, **plus a seventh, targeted audit pass** (PR review round 7) independently confirming the current 7-entry bundle — after a corroborating entry was added post-audit in PR review round 4 — remains internally consistent and verdict-unchanged, after fixing 6 real defects found in the process; under either scope reading, none of the seven passes was ever dispatched to check, as a discrete item, whether the recorded network-denial mechanism meets the isolation contract's strongest-available-mechanism rule, or whether a given invocation meets FR-011's literal requirement — a gap in what was dispatched, parallel to the T033 original-decomposition gap discussed in that same evidence index, not a failure of the action actually dispatched; see [`specs/008-spec-kit-hook-viability/checklists/evidence-index.md`](specs/008-spec-kit-hook-viability/checklists/evidence-index.md)'s "Independent audit" section for the full account); this audit is a fresh-context LLM consistency review of the session-scoped evidence bundle, **not** ADR-0014's rung-2 `reference-verified` maturity state (that term is reserved for reproducible, self-verifying, fail-closed evidence from a maintainer-owned isolated reference repository — the bar Phase 6 met — which this spike does not claim); tracked, sanitized FR-024 evidence index at [`specs/008-spec-kit-hook-viability/checklists/evidence-index.md`](specs/008-spec-kit-hook-viability/checklists/evidence-index.md), which documents a discovered T005 conformance gap (rank-1 network-denial mechanism was available but not selected for the recorded lifecycle invocations) and, per PR review round 11, a resulting T012 checkpoint gap (T012's own action is dependency-set certification, which cannot be true of a checkpoint naming T005 among its dependencies while T005 remains unchecked); per PR review, T005 and T012 are marked incomplete rather than qualified-while-checked, since T005's own mechanism-selection did not meet contract and T012 cannot certify a dependency set containing an unchecked member; this does not invalidate the invocations T005 gated (they ran and are fully evidenced under rank 3), does not change the `no-go` verdict, which is driven independently by the mutation baseline; per PR review round 13 (a comment initially missed in triage, surfaced via a later unresolved-threads sanity sweep), FR-011's own network-disabled requirement was not, in the strict sense, met by the `hook-fire` invocation specifically (T024; the only invocation among those T005 gated backed by a real, successful adrkit-CLI subprocess call) — this is the same T005 gap traced to its FR-011-specific consequence, not a new defect, and T024's own checkbox remains checked since its own defined action (capture and honestly record which mechanism ran) was performed correctly (see [`specs/008-spec-kit-hook-viability/checklists/evidence-index.md`](specs/008-spec-kit-hook-viability/checklists/evidence-index.md)'s "Network / credential limits" section); **not** externally validated, **not** released/adopted, and did **not** change Phase 6's own landed/reference-verified status in either direction |
