...
|---|---|---|
| Spec Kit | `packages/adapters/spec-kit/` (`@adrkit/spec-kit`) | **shipped, rung 1 only** — three namespaced commands (`context`, `check`, `draft`) plus one optional `after_plan` hook, pinned to Spec Kit `>=0.13.0,<0.16.0` (installed and rendered against 0.13.0, 0.14.4, and 0.15.1). Authorized by [ADR-0019](docs/adr/0019-ship-the-spec-kit-extension-treating-the-spike-no-go-as-a-measurement-artifact.md), which records spike 008's `no-go` as a measurement artifact of that spike's own verdict procedure (byte-identical `git status` applied to `install`/`remove`, lifecycle actions that necessarily write) rather than a finding against the mechanism, and carries the spike's verified constraints forward as production requirements. Spike 008's verdict, evidence bundle, and audit history are unmodified. Verified by real installs into isolated scratch projects on live upstream, but those artifacts were session-scoped, untracked, and unreviewed, so this stays rung 1: **not** reference-verified, **not** externally validated ([ADR-0014](docs/adr/0014-stage-phase-landing-evidence-across-a-three-rung-validation-ladder.md) rungs 2–3 open). Landing it made ADR-0007's `core-has-no-adapter-deps` assertion non-vacuous for the first time; the rule has since been observed failing against a deliberately introduced `@adrkit/core → @adrkit/spec-kit` dependency ([ADR-0016](docs/adr/0016-require-every-check-to-be-observed-failing-before-it-counts-as-coverage.md)). |
