...
5. [x] Submit the catalog entry to `github/spec-kit` once a release asset exists — landed 2026-08-25 via [github/spec-kit#3947](https://github.com/github/spec-kit/pull/3947); adrkit is listed in the community catalog. **This is a standing per-release obligation, not a one-time task, and treating it as one-time cost two releases:** the entry stayed pinned at 0.1.2 through 0.1.3 and 0.1.4, still advertising `>=0.13.0,<0.16.0` — a bound that by then refused to install on any current Spec Kit. Refreshed for 0.1.4 on 2026-09-13 via [github/spec-kit#4571](https://github.com/github/spec-kit/issues/4571). Upstream now refuses direct pull requests against `catalog.community.json`; updates go through the Extension Submission issue template. The release runbook carries the step ([`docs/RELEASING.md`](../RELEASING.md)).
