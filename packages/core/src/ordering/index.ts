/**
 * @adrkit/core — the one locale-independent comparator and the canonical orderings
 * every first-party channel uses. Never `String.prototype.localeCompare` (research §R6).
 *
 * Promoted verbatim from `@adrkit/mcp` (`packages/mcp/src/corpus/ordering.ts`) so the
 * queue kernel and the MCP corpus projection share a single byte-stable ordering source.
 */
